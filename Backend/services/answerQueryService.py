...
async def answerQuery(video_id, user_question):
    ...
    if result and result["transcript_found"]:
        ...
        print(f"Video {video_id} found in pinr code database.")
        docs = getresultfromPinecodeDB(video_id, user_question)
        return llm.answer_query(docs, user_question)
        ...
        print("Message published to Queue for embedding generation")

        return {"message": "Embeddings generation in progress for the given video ID. Please try again later."}
